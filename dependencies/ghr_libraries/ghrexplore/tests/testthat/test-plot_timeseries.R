...
# Test case: 'panel' and 'highlight' cannot be specified together
test_that("'panel' and 'highlight' cannot be specified together", {
  data <- data.frame(time = as.Date("2025-01-01"), a = 1)
  expect_error(plot_timeseries(data = data, time = "time",var = "a",
                               panel = TRUE, highlight = "a"),
               "'panel' and 'highlight' cannot be specified together")
})
...
test_that("Duplicated time units in a single time series", {
  ...
  expect_error(plot_timeseries(data, var = "value", time = "time"),
               "Duplicated time units detected, if data contains multiple spatial")
})
...
test_that("Correct plot for single time series", {
  ...
  plot_result <- plot_timeseries(data, var = "value", time = "time", panel = FALSE)
...
test_that("Correct title generation", {
  ...
  plot_result <- plot_timeseries(data, var = "value", time = "time", area = "area")
  expect_equal(plot_result$labels$title, "value")
})
...
test_that("Correct time aggregation handling", {
  data<- data.frame(
    time = as.Date(c("2021-01-01","2021-01-02","2021-02-01","2021-02-02")),
    var = c(10, 30, 20, 40)
  )
...
test_that("Highlight functionality for areas", {
  ...
  red_line_found <- "red" %in% plot_build$plot$layers[[2]]$computed_geom_params$colour
  
  # Expect that the red line is found
  expect_true(red_line_found)
  })
  

# Test case: Panel = TRUE for multiple areas
test_that("Correct handling for panel = TRUE", {
  data <- data.frame(time = as.Date(c('2025-01-01', '2025-01-02',
                                      '2025-01-01', '2025-01-02')), 
                     value = c(1,2,3,1), 
                     area = rep(letters[1:2], each = 2))
...
test_that("Correct time aggregation based on 'aggregate_time' - month", {
  data <- data.frame(time = as.Date(c("2022-01-01", "2022-01-08", "2022-02-01")),
                     cases = c(100, 150, 200), pop = c(1000, 1000, 1000))
...
test_that("Highlight option works correctly", {
  data <- data.frame(time = as.Date(c('2025-01-01', '2025-01-02',
                                      '2025-01-01', '2025-01-02')), 
                     cases = c(1,2,3,4), pop = 1000,
                     area = rep(letters[1:2], each = 2))

  
  # Expect warning when highlight is specified with 'counts' type
  expect_warning(plot_timeseries(data = data, time = "time", var = "cases",
                                 area = "area", highlight = "a", type = "counts"), 
                 "Highlight not available for counts")
  
  # Test the highlight with a valid type
  result <- plot_timeseries(data = data, time = "time", var = "cases", pop="pop",
                            area = "area", highlight = "a", type = "inc")
  
  expect_s3_class(result, "gg")  # Ensure it's a ggplot object
...
test_that("Panels adjust properly based on area", {
  data <- data.frame(time = as.Date(c('2025-01-01', '2025-01-02',
                                      '2025-01-01', '2025-01-02')), 
  ...
  result <- plot_timeseries(data = data, time = "time", var = "cases",
                            area = "area", type = "counts", panel = TRUE)
