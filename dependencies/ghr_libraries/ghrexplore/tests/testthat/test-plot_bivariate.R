...
test_that("works with panels", {
  df <- data.frame(a = rnorm(20), b = rnorm(20), region = rep(c("X", "Y"), 10))
  p <- plot_bivariate(df, var = c("a", "b"), area = "region", panel = TRUE)
  expect_s3_class(p, "ggplot")
})

test_that("free_x_scale and free_y_scale work", {
  df <- data.frame(a = rnorm(20), b = rnorm(20), region = rep(c("X", "Y"), 10))
  expect_s3_class(plot_bivariate(df, var = c("a", "b"), area = "region", panel = TRUE, free_x_scale = TRUE), "ggplot")
  expect_s3_class(plot_bivariate(df, var = c("a", "b"), area = "region", panel = TRUE, free_y_scale = TRUE), "ggplot")
  expect_s3_class(plot_bivariate(df, var = c("a", "b"), area = "region", panel = TRUE, free_x_scale = TRUE, free_y_scale = TRUE), "ggplot")
})
