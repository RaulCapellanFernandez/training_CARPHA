...
test_that("get_time_interval requires no duplicate time if group is NULL", {
  df_dup <- data.frame(
...
test_that("get_time_interval can handle multiple groups (no error if duplicates across groups)", {
  df_groups <- data.frame(
    group = c("A","A","B","B"),
    time = as.Date(c("2021-01-01", "2021-01-02", "2021-01-01", "2021-01-02"))
  )
  # No error should occur, because we specified `group`
  interval <- get_time_interval(df_groups, time = "time", group = "group")
...
test_that("check_consecutive identifies consecutive daily data (no group)", {
  df_daily <- data.frame(
    time = c("2021-01-01","2021-01-02","2021-01-03")
  )
...
test_that("check_consecutive returns FALSE if daily data is missing a date", {
  ...
  # Not consecutive
  expect_false(check_consecutive(df_skip, time = "time"))
})

test_that("check_consecutive works with multiple groups", {
  df_group <- data.frame(
    group = c("A","A","B","B"),
    time = as.Date(c("2021-01-01","2021-01-02","2021-01-01","2021-01-02"))
  )
  # Both A and B are consecutive daily
  expect_true(check_consecutive(df_group, time = "time", group = "group"))
})

test_that("check_consecutive throws error if duplicates exist and group not provided", {
  df_dup <- data.frame(
